...
#define INER 0.05
static uint16_t inertial_filter(uint16_t val)
{
    static uint16_t last_val = 0;
    uint16_t ret_val = 0;
    ret_val = INER * val + (1-INER) * last_val;
    last_val = ret_val;
    return ret_val;
}

void AutoSetBacklight(uint16_t light_adc,uint8_t* Brigh) 
{
  uint8_t backlight_level = 0;
  light_adc = inertial_filter(light_adc);
  if(light_adc<=400)
  {
    backlight_level=10;
  }
  else if(light_adc>400 && light_adc<=800)
  {
    backlight_level=20;
  }
  else if(light_adc>800 && light_adc<=1200)
  {
    backlight_level=30;
  }
  else if(light_adc>1200 && light_adc<=1600)
  {
    backlight_level=40;
  }
  else if(light_adc>1600 && light_adc<=2000)
  {
    backlight_level=50;
  }
    else if(light_adc>2000 && light_adc<=2400)
  {
    backlight_level=60;
  }
  else if(light_adc>2400 && light_adc<=2800)
  {
    backlight_level=70;
  }
  else if(light_adc>2800 && light_adc<=3200)
  {
    backlight_level=80;
  }
    else if(light_adc>3200 && light_adc<=3600)
  {
    backlight_level=90;
  }
  else if(light_adc>3600)
  {
    backlight_level=100;
  }
 *Brigh = (100-backlight_level);
   TRACE("Backlight level:%d\n",backlight_level);
}