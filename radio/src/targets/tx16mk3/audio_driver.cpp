...
int audioInit()
{
  ...
  if (tas2505_probe(&_tas2505, I2C_Bus_2, 0x30 >> 1) < 0) {
    return -1;
  }
...
{
    Set_Speaker_power(&_tas2505,1);
}

void disableSpeaker(void)
{
    Set_Speaker_power(&_tas2505,0);
}
void enableHeadphone(void) 
{ 
    Set_Headphone_power(&_tas2505, 1); 
}
void disableHeadphone(void) 
{ 
    Set_Headphone_power(&_tas2505,0); 
}